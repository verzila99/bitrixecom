...
<div class="bx-subscribe" id="sender-subscribe">
					...
					<table>
						<tr>
							<td style="padding-right: 40px; padding-bottom: 0px;">
								<img src="<?= ($this->GetFolder(
									) . '/images/' . ($arResult['MESSAGE']['TYPE'] == 'ERROR' ? 'icon-alert.png'
										: 'icon-ok.png')) ?>" alt="">
							</td>
							<td>
								<div style="font-size: 22px;"><?= GetMessage(
										'subscr_form_response_' . $arResult['MESSAGE']['TYPE']
									) ?></div>
								<div style="font-size: 16px;"><?= htmlspecialcharsbx(
		...
		(function () {
			...
			function mailSender() {
				setTimeout(function () {
					...
					btn_span.outterHTML = "<span><i class='fa fa-check'></i> <?=GetMessage(
						"subscr_form_button_sent"
					)?></span>";
	...
	<form id="bx_subscribe_subform_<?= $buttonId ?>"
				role="form"
				method="post"
			...
			<?
				foreach ($arResult["RUBRICS"] as $itemID => $itemValue): ?>
					<div class="bx_subscribe_checkbox_container">
						...
						<label for="SENDER_SUBSCRIBE_RUB_ID_<?= $itemValue["ID"] ?>"><?= htmlspecialcharsbx(
								$itemValue["NAME"]
							) ?></label>
					</div>
				<?
		...
		<div class="bx_subscribe_submit_container">
			<button class="btn btn-outline-secondary" id="bx_subscribe_btn_<?= $buttonId ?>">
