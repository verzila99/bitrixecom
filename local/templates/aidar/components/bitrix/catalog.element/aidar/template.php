<?php
	...
	$buyButtonClassName = in_array('BUY', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? 'btn-primary'
		: 'btn-link';
	$showAddBtn = in_array('ADD', $arParams['ADD_TO_BASKET_ACTION']);
	$showButtonClassName = in_array('ADD', $arParams['ADD_TO_BASKET_ACTION_PRIMARY']) ? 'btn-primary'
		: 'btn-link';
...
?>
	<div class="bx-catalog-element<?= $themeClass ?>"
			 id="<?= $itemIds['ID'] ?>"
			 itemscope
		...
		<div class="row">
									...
									<div class="product-item-detail-slider-controls-block"
										...
										<?php
											foreach ($offer['MORE_PHOTO'] as $keyPhoto => $photo) {
												?>
												<div class="product-item-detail-slider-controls-image<?= ($keyPhoto == 0
													? ' active' : '') ?>"
								...
								<div class="product-item-detail-slider-controls-block"
									...
									<?php
										if (!empty($actualItem['MORE_PHOTO'])) {
											foreach ($actualItem['MORE_PHOTO'] as $key => $photo) {
												?>
												<div class="product-item-detail-slider-controls-image<?= ($key == 0
													? ' active' : '') ?>"
			...
			<?php
				...
				$mainBlockProperties = array_intersect_key(
					$arResult['DISPLAY_PROPERTIES'],
					$arParams['MAIN_BLOCK_PROPERTY_CODE']
				);
														...
														<?php
															foreach ($arResult['SKU_PROPS'] as $skuProperty) {
																...
																<div data-entity="sku-line-block" class="mb-3">
																	<div class="product-item-scu-container-title"><?= htmlspecialcharsEx(
																			$skuProperty['NAME']
																		) ?></div>
																	...
																	<?php
																		foreach ($mainBlockProperties as $property) {
																			...
																			<li class="product-item-detail-properties-item">
																				...
																				<span class="product-item-detail-properties-value"><?= (is_array(
								...
								foreach ($arParams['PRODUCT_PAY_BLOCK_ORDER'] as $blockName) {
									switch ($blockName) {
										case 'price':
											...
											<p class="goToDescription text-center">
												<a href="#details" class="scroll-to ">Перейти к описанию и
																															характеристикам
												</a>
											</p>


											<h2 class=" text-center price mb-1"
													id="<?= $itemIds['PRICE_ID'] ?>"><?= $price['PRINT_RATIO_PRICE'] ?></h2>
										...
										case 'priceRanges':
											if ($arParams['USE_PRICE_COUNT']) {
												$showRanges = !$haveOffers && count(
														$actualItem['ITEM_QUANTITY_RANGES']
													) > 1;
												...
												(<?= (Loc::getMessage(
																		...
																		array(
																			'#RATIO#' => ($useRatio ? $measureRatio
																					: '1') . ' ' . $actualItem['ITEM_MEASURE']['TITLE']
																		)
																	)) ?>)
											</span>
															</div>
															...
															if ($showRanges) {
																...
																<span class="product-item-detail-properties-name text-muted">
																	...
																	?>
																				...
																				<span class="product-item-detail-properties-dots"></span>
																				<span class="product-item-detail-properties-value"><?= ($useRatio
																						? $itemPrice['PRINT_RATIO_PRICE']
																						: $itemPrice['PRINT_PRICE']) ?></span>
																			</li>
																			<?php
																		}
																	}
												...
												<div class="mb-3" <?= (!$actualItem['CAN_BUY'] ? ' style="display: none;"'
													...
													<?php
														if (Loc::getMessage('CATALOG_QUANTITY')) {
															?>
															<div class="product-item-detail-info-container-title text-center"><?= Loc::getMessage(
																	'CATALOG_QUANTITY'
																) ?></div>
															<?php
														}
								...
								<div class="product-item-detail-tab-content active"
									...
									<?php
										if (
											...
											echo $arResult['PREVIEW_TEXT_TYPE'] === 'html' ? $arResult['PREVIEW_TEXT']
								...
								<div class="product-item-detail-tab-content"
									...
									<?php
										...
										if ($arResult['SHOW_OFFERS_PROPS']) {
											?>
											<ul class="product-item-detail-properties"
													id="<?= $itemIds['DISPLAY_PROP_DIV'] ?>"></ul>
											<?php
										}
									?>
								</div>
		...
		<div class="row">
			<div class="col">
				<?php
					if ($arResult['CATALOG'] && $actualItem['CAN_BUY'] && \Bitrix\Main\ModuleManager::isModuleInstalled(
							'sale'
						)) {
						$APPLICATION->IncludeComponent(
					...
					if ($arResult['CATALOG'] && $arParams['USE_GIFTS_DETAIL'] == 'Y' && \Bitrix\Main\ModuleManager::isModuleInstalled(
						...
						<div data-entity="parent-container">
							<?php
								if (!isset($arParams['GIFTS_DETAIL_HIDE_BLOCK_TITLE']) || $arParams['GIFTS_DETAIL_HIDE_BLOCK_TITLE'] !== 'Y') {
									...
									<div class="catalog-block-header"
										...
										<?= ($arParams['GIFTS_DETAIL_BLOCK_TITLE']
											?: Loc::getMessage(
												'CT_BCE_CATALOG_GIFT_BLOCK_TITLE_DEFAULT'
											)) ?>
									</div>
									<?php
								}
								...
								$APPLICATION->IncludeComponent(
									'bitrix:sale.products.gift', 'bootstrap_v4', array(
									'CUSTOM_SITE_ID' => $arParams['CUSTOM_SITE_ID'] ?? null,
					...
					if ($arResult['CATALOG'] && $arParams['USE_GIFTS_MAIN_PR_SECTION_LIST'] == 'Y' && \Bitrix\Main\ModuleManager::isModuleInstalled(
							'sale'
						)) {
						?>
						<div data-entity="parent-container">
							<?php
								if (!isset($arParams['GIFTS_MAIN_PRODUCT_DETAIL_HIDE_BLOCK_TITLE']) || $arParams['GIFTS_MAIN_PRODUCT_DETAIL_HIDE_BLOCK_TITLE'] !== 'Y') {
									...
									<div class="catalog-block-header"
										...
										<?= ($arParams['GIFTS_MAIN_PRODUCT_DETAIL_BLOCK_TITLE']
											?: Loc::getMessage(
												'CT_BCE_CATALOG_GIFTS_MAIN_BLOCK_TITLE_DEFAULT'
											)) ?>
									</div>
									<?php
								}
								...
								$APPLICATION->IncludeComponent(
									'bitrix:sale.gift.main.products', 'bootstrap_v4',
									array(
										'CUSTOM_SITE_ID' => $arParams['CUSTOM_SITE_ID'] ?? null,
		...
		<div class="p-2 product-item-detail-short-card-fixed d-none d-md-block"
			...
			<div class="product-item-detail-short-card-content-container">
				...
				<div class="product-item-detail-short-card-price d-flex align-items-center">
					<?php
					...
					?>
					<h3 class="price"
							data-entity="panel-price"><?= $price['PRINT_RATIO_PRICE'] ?></h3>
				</div>
				<?php
					if ($showAddBtn) {
						?>
						...
						foreach ($jsOffer['ITEM_QUANTITY_RANGES'] as $range) {
							if ($range['HASH'] !== 'ZERO-INF') {
								...
								if ($itemPrice) {
									...
									$strPriceRanges .= '</dt><dd>' . ($useRatio ? $itemPrice['PRINT_RATIO_PRICE']
														...
														if (
															...
															foreach ($propInfo['VALUES'] as $valueId => $value) {
																...
																<label>
																	<input type="radio"
																				 name="<?= $arParams['PRODUCT_PROPS_VARIABLE'] ?>[<?= $propId ?>]"
																				 value="<?= $valueId ?>" <?= ($valueId == $propInfo['SELECTED']
																		? '"checked"' : '') ?>>
																	<?= $value ?>
																</label>
																<br>
	...
	<script>
		BX.message({
			...
			BTN_MESSAGE_DETAIL_BASKET_REDIRECT: '<?=GetMessageJS(
